...
import fitIn.fitInserver.config.auth.dto.OAuthAttributes;
import fitIn.fitInserver.config.auth.dto.SessionAccount;
import fitIn.fitInserver.domain.Account;
import fitIn.fitInserver.domain.AccountRepositorys;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    private final AccountRepositorys accountRepositorys;
    ...
    private Account saveOrUpdate(OAuthAttributes attributes) {
        Account account = accountRepositorys.findByEmail(attributes.getEmail())
